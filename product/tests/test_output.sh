...
t::assert_contains "$msg" "2 report"
...
report::upload "$csv" >"$tmpout" 2>&1
rc=$?
t::check "upload: dashboard success -> rc 0" "[[ $rc -eq 0 ]]"
t::assert_contains "$(cat "$tmpout")" "Synced to tScrub dashboard"
...
t::assert_contains "$(cat "$tmpout")" "Dashboard upload failed"
t::check "upload: dashboard fail does not claim missing token" "[[ \"$(cat "$tmpout")\" != *'no API token'* ]]"
...
report::upload "$csv" >"$tmpout" 2>&1
rc=$?
t::check "upload: URL without token -> rc 1" "[[ $rc -eq 1 ]]"
t::assert_contains "$(cat "$tmpout")" "no API token"
t::assert_eq "no API token" "${REPORT_DASH_REASON:-}" "no-token reason"
...
t::assert_contains "$(cat "$tmpout")" "falling back to ftp"
unset FAKE_CURL_FAIL
...
rm -rf "$out" "$tdir" "$FAKE_CURL_LOG" "$FAKE_LFTP_LOG" "$LOG_FILE" "$tmpout"
t::summary
