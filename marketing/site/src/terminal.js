...
const DRIVES = [
  ...
  { dev: 'sda', kind: 'SATA HDD', method: 'Enhanced Erase', cert: 'SANITISATION' },
  { dev: 'sdb', kind: 'SCSI SSD', method: 'nwipe Quick', cert: 'CLEAR' }
]
...
async function runDemo(dryRun = false) {
  ...
  el('t-muted', 'tScrub v1.4.6 — verifiable disk sanitisation')
  el('')
  ...
  el('t-ok', '  sda      → Enhanced Erase      (PURGE)')
  ...
  el('')
  el('t-ok', '✓ Complete. Report written to /tScrub_48213_20260919T103000Z.csv')
  el('t-muted', '  Chain of custody ID: 48213')
  el('')
  busy = false
}
